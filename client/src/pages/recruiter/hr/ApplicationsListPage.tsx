  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 text-base sm:text-sm">{subtitle}</p>
        </div>
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-base sm:text-sm"
          >
        ...
        {error && (
          <div className="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-base sm:text-sm text-red-700">
            {error}
          </div>
        )}
        ...
        {applications.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg shadow">
            ...
            <h3 className="mt-2 text-base sm:text-sm font-medium" style={{ color: 'var(--hh-text)' }}>
              {isScopedView ? 'No applications yet' : 'No applications submitted yet'}
            </h3>
            <p className="mt-1 text-base sm:text-sm" style={{ color: 'var(--hh-text-secondary)' }}>
              {isScopedView
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
                    ...
                    <tr key={app.id} className="hover:bg-gray-50">
                      {!isScopedView && (
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                          <div className="text-sm sm:text-base font-medium text-gray-900">{app.requisition_title || 'N/A'}</div>
                          <div className="text-xs sm:text-sm text-gray-500">{app.department || app.location || 'Requisition'}</div>
                        </td>
                      )}
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div className="text-sm sm:text-base font-medium text-gray-900">{formatCandidateName(app)}</div>
                        <div className="text-xs sm:text-sm text-gray-500">{app.candidate_email || app.candidate_id || 'N/A'}</div>
                      </td>
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div className="text-sm sm:text-base text-gray-500">{formatDate(app.submitted_at)}</div>
                      </td>
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
            ...
            {filteredApplications.length === 0 && applications.length > 0 && (
              <div className="text-center py-8 text-gray-500 text-base sm:text-sm">
